...
const API_BASE_URL = '/api/v1/tipos-movimiento';
    ...
    const renderForm = () => (
        <div style={{ backgroundColor: 'white', borderRadius: '8px', border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.04)', padding: '32px', maxWidth: '800px' }}>
            ...
            <div style={{ display: 'grid', gap: '24px' }}>
                <div>
                    <label style={{ display: 'block', fontSize: '12.5px', fontWeight: '600', color: '#475569', marginBottom: '6px', letterSpacing: '0.1px' }}>Nombre *</label>
                ...
                <div>
                    <label style={{ display: 'block', fontSize: '12.5px', fontWeight: '600', color: '#475569', marginBottom: '6px', letterSpacing: '0.1px' }}>Descripción *</label>
            ...
            <div style={{ flex: 1, overflow: 'auto', padding: '24px', backgroundColor: '#f8fafc' }}>
                ...
                {loading && viewMode === 'list' ? (
                                ...
                                <tbody>
                                    {filteredTipos.length === 0 ? (
                                        ...
                                        filteredTipos.map((tipo) => (
                                            <tr key={tipo.id} style={{ borderBottom: '1px solid #f1f5f9', cursor: 'pointer' }}
                                                onClick={() => handleViewTipo(tipo)}
                                                onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#f8fafc'}
                                                onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'transparent'}>
                                                <td style={{ padding: '13px 20px', fontSize: '13.5px', fontWeight: '500' }}>{tipo.nombre}</td>
                                                <td style={{ padding: '13px 20px', fontSize: '13.5px', color: '#6b7280' }}>{tipo.descripcion}</td>
                                                <td style={{ padding: '13px 20px' }} onClick={(e) => e.stopPropagation()}>
                                                    <div style={{ display: 'flex', gap: '8px' }}>
                                                        <button onClick={() => handleViewTipo(tipo)} style={{ padding: '5px', color: '#16a34a', background: 'transparent', border: 'none', cursor: 'pointer', borderRadius: '4px', display: 'inline-flex', alignItems: 'center' }} title="Ver"><Eye size={16} /></button>
                                                        <button onClick={() => handleEdit(tipo)} style={{ padding: '5px', color: '#1d4ed8', background: 'transparent', border: 'none', cursor: 'pointer', borderRadius: '4px', display: 'inline-flex', alignItems: 'center' }} title="Editar"><Edit2 size={16} /></button>
                                                        <button onClick={() => handleDelete(tipo)} style={{ padding: '5px', color: '#dc2626', background: 'transparent', border: 'none', cursor: 'pointer', borderRadius: '4px', display: 'inline-flex', alignItems: 'center' }} title="Eliminar"><Trash2 size={16} /></button>
                                                    </div>
                                                </td>
                                            </tr>
                                        ))
                    ...
                    <div style={{ backgroundColor: 'white', borderRadius: '8px', border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.04)', padding: '32px', maxWidth: '896px' }}>
                        ...
                        <div style={{ display: 'grid', gap: '24px' }}>
                            <div>
                                <h3 style={{ fontSize: '11px', fontWeight: '600', color: '#94a3b8', textTransform: 'uppercase', marginBottom: '6px', letterSpacing: '0.6px' }}>Nombre</h3>
                                <p style={{ fontSize: '15px', margin: 0, color: '#1e293b' }}>{selectedTipo?.nombre}</p>
                            </div>
                            <div>
                                <h3 style={{ fontSize: '11px', fontWeight: '600', color: '#94a3b8', textTransform: 'uppercase', marginBottom: '6px', letterSpacing: '0.6px' }}>Descripción</h3>
                                <p style={{ fontSize: '15px', margin: 0, color: '#1e293b', lineHeight: '1.6' }}>{selectedTipo?.descripcion}</p>
                            </div>
                        </div>
                        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', marginTop: '32px', paddingTop: '24px', borderTop: '1px solid #e5e7eb' }}>
                            ...
                            <button onClick={() => handleEdit(selectedTipo)} style={{ padding: '7px 14px', backgroundColor: '#1d4ed8', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: '500', fontSize: '13px' }}>Editar Tipo</button>
                        </div>
                    </div>
                ) : renderForm()}
            </div>
